...
const fileOptions = {
    ...
    checkChunkUploadedByResponse: function (chunk, message) {
        ...
        return (objMessage.data.uploadedChunkNumberList || []).indexOf(chunk.offset + 1) >= 0
    },
    maxChunkRetries: 0,
...
const fileUploaded = (rootFile, file, message, chunk) => {
    ...
    if (res.code === 0) {
        if (res.data) {
            if (res.data.merge) {
                doMerge(file)
            } else if (res.data.uploadedChunkNumberList && res.data.uploadedChunkNumberList.length === file.chunks.length) {
                doMerge(file)
            }
        } else {
            ElMessage.success('文件：' + file.name + ' 上传完成')
