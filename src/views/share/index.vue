...
const refreshShareInfo = (data) => {
    ...
    tableData.value = data.userFileList
}
    ...
    userService.infoWithoutPageJump(res => {
        if (res.code === 0) {
            shareService.getSimpleShareDetail({
                ...
                if (res.code === 0) {
                    let url = panUtil.getUrlPrefix() + '/share/file/save?fileId=' + item.fileId.replace(/\+/g, '%2B') + '&shareToken=' + getShareToken() + '&authorization=' + getToken(),
